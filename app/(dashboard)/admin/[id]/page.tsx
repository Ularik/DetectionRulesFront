...
export default function RuleTable() {
  ...
  const { data: rule, isPending, isError, error, refetch } = useOneRule(id as string);
  ...
  if (!rule) {
    return (
      <div className="text-center py-20 text-white">Правило не найдено</div>
    );
  }

  return (
    <div className="mx-4 my-6 rounded-2xl border shadow-sm overflow-hidden max-w-[900px]">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead className="w-[220px] bg-muted/30 text-center">
              Поле
            </TableHead>
        ...
        <TableBody>
          {(Object.keys(rule) as string[]).map((key, i) => (
            <TableRow key={i} className="hover:bg-muted/20">
              <TableCell className="font-medium text-muted-foreground text-sm text-center">
                {key}
              </TableCell>
              <TableCell className="max-w-[300px] text-start">
                <div
